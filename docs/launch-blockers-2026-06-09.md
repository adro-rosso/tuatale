...
| C6 | **Provider resilience — single image-gen provider, on a *preview* model + AI-Studio key.** All generation depends on one Google endpoint (`gemini-3.1-flash-image-preview` via a consumer `GEMINI_API_KEY`). Observed 2026-07-07: repeated multi-minute stalls where even a trivial image hangs >50s (the GA `gemini-2.5-flash-image` was ~8s in the same window). Fail-fast+retry+hedge (worker `0348fcd`) makes stalls degrade gracefully (fail at 135s, not a 300s hang) but cannot conjure an image while the provider is down. | **At volume:** consumer-key rate limits (429s) + preview-model latency/outages → the interactive **pre-sale preview** fails for stretches (lost conversions); the **paid book path is structurally protected** (see framing note). | **SCALE gate** (not a first-sale blocker) | Large (bake-off → Vertex → fallback) | Vertex/API + spend |

**Scale-gate framing (C6).** This is a *volume* gate, not a first-sale blocker, and the exposure is asymmetric:
- **Paid customers are structurally protected.** Book generation is **async** + wrapped by the reliability cluster (R1 completeness gate, R2 recovery, R3 resume/checkpoint, D2 fatal-stop, charged-then-failed recovery). A provider stall means a book is **delayed and retried in the background, not lost, with no bad charge** — the customer waits for an email, not a spinner. Even a multi-hour outage is "books are slow today," not "paid customers got nothing."
- **The pre-sale preview is the exposed surface** — interactive, so a stall shows as "busy/try again." It's optional and pre-purchase (no money taken), so the cost is a degraded first impression / possible lost conversion, not a failed paid order.
- **Today's severity is a prototype-config artifact,** not a representative production condition: we run a *preview* model on a *consumer* AI-Studio key. Production requires a **GA model on Vertex AI** (provisioned throughput + SLA + real quota) — and *volume itself forces that move* (consumer keys rate-limit at scale). Full workstream: **[docs/provider-resilience-plan.md](provider-resilience-plan.md)** (GA bake-off → Vertex → fallback ladder → provider-health alerting).
