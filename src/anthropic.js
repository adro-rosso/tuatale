...
- \`action\` is for the illustrator. It must be VISUALLY CONCRETE — a specific moment an illustrator can draw without inventing details. "Lila kneels in the tall grass cupping a firefly in her hands" is drawable. "Lila feels wonder" is not. Pick the most picture-able moment in each scene. Always show the protagonist in the action. SCREENS AND INTERFACES: the illustrator renders any UNSPECIFIED text on a screen or interface as gibberish letterforms — it cannot invent a real, readable menu or UI. So do NOT write an action whose focus is reading or operating unspecified on-screen text: no "scrolling through the streaming menu", "reading the news on his phone", "an open laptop full of code". If such a device belongs in the scene, describe it by its GLOW, or by what the character is DOING or FEELING with it, NOT by its interface content — write "the television's blue glow on his face as he sinks into the sofa", not "navigating the streaming menu". This NARROWS the demand, it does not ban the prop: the screen can still be in the picture. EXCEPTION — SHORT text you write out in full renders correctly and is welcome where it matters: a sign reading "ROAD CLOSED", a banner reading "HAPPY BIRTHDAY". The rule bites only on screen/interface text you have NOT pinned to specific words.
